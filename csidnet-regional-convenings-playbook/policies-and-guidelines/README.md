# Policies & Guidelines

**Code of Conduct: Reporting & Escalation** _(updated on 21 July 2026)_

**Points of contact:** Every regional event has a confirmed in-person point of contact for Code of Conduct issues. Please introduce this person on the first day to participants alongside the [Code of Conduct](../../readme/network-policies/code-of-conduct.md) itself.

**2026 CoC Points of Contact**

<table data-header-hidden data-search="false"><thead><tr><th></th><th></th><th></th></tr></thead><tbody><tr><td><strong>Location</strong></td><td><strong>Name</strong></td><td><strong>Email</strong></td></tr><tr><td>Thailand</td><td>Vidhyakorn Mahd-Adam</td><td><a href="mailto:Vidhyakorn@tropmedres.ac">Vidhyakorn@tropmedres.ac</a></td></tr><tr><td>Bangladesh</td><td>Mohammad Rezaul Karim</td><td><a href="mailto:MOHAMMAD.KARIM@icddrb.org">MOHAMMAD.KARIM@icddrb.org</a> </td></tr><tr><td>Kenya</td><td><p>Maureen Mbithe</p><p>Linet Onyina</p><p>Umazi Munyikah</p></td><td><a href="mailto:analytics.officer@africabioethicsnetwork.org">analytics.officer@africabioethicsnetwork.org</a>, <a href="mailto:researchoffice@africabioethicsnetwork.org">researchoffice@africabioethicsnetwork.org</a>, <a href="mailto:programs@africabioethicsnetwork.org">programs@africabioethicsnetwork.org</a> </td></tr><tr><td>Colombia</td><td>Diana Fajardo</td><td><a href="mailto:dclemenciaf@gmail.com">dclemenciaf@gmail.com</a> </td></tr><tr><td>Cameroon</td><td>Ms. Nkahe Diane Leslie</td><td><a href="mailto:leslie.diane112@gmail.com">leslie.diane112@gmail.com</a> </td></tr><tr><td>Advisory Committee (based in Thailand)</td><td>Nishanta Sharma</td><td><a href="mailto:nishanta.s@hitap.net">nishanta.s@hitap.net</a> </td></tr></tbody></table>

**Reporting flow at a glance:**\
An incident or concern can be raised in person, by phone, by email, or via the [reporting form](https://forms.gle/3FsyDmG2rm1bF9PF7). It goes to the event's point of contact first (see contacts above), who follows the in-person reporting template below to capture details and immediate response, and escalates further depending on severity (see triage matrix below.)

<figure><img src="../../.gitbook/assets/CoC_Escalation_Flow.png" alt=""><figcaption></figcaption></figure>

**Risk-tiered triage matrix**

| Risk Level            | Typical Incidents                                                                                                                                                                           | Immediate Response (within 24 hrs)                                                                                       | Decision Path                                                                                               | Target Timeline                                           |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| 🔴 Level 1 – Critical | Physical assault, sexual harassment/assault, credible threats of violence, safeguarding concerns involving children/vulnerable persons, stalking, serious discrimination, medical emergency | Ensure immediate safety; notify designated CoC Lead; separate parties if necessary; document incident; preserve evidence | Immediate escalation to CoC Committee/Leadership; formal investigation; external authorities where required | Acknowledged <2 hrs; action initiated within 24 hrs       |
| 🟠 Level 2 – High     | Bullying, intimidation, repeated harassment, severe verbal abuse, retaliation, hate speech, deliberate breach of confidentiality                                                            | Acknowledge report; assess immediate risks; interview reporter; notify CoC Lead; document                                | Formal review/investigation by designated panel                                                             | Initial review within 24 hrs; investigation within 48 hrs |
| 🟡 Level 3 – Moderate | Inappropriate language, disrespectful behaviour, interpersonal conflict, isolated discriminatory remarks, unprofessional conduct                                                            | Acknowledge; gather facts; determine informal resolution vs. formal review                                               | Managed by CoC Lead, possible mediation/coaching                                                            | Response within 24 hrs; resolution plan within 5–10 days  |
| 🟢 Level 4 – Low      | Minor misunderstandings, first-time minor issues, communication concerns, etiquette                                                                                                         | Record concern; provide guidance/coaching; monitor for recurrence                                                        | Informal resolution                                                                                         | Within 5 working days                                     |

**In-person reporting guide (for the on-site point of contact):**\
Print a few copies of the reporting template below to be able to capture any in-person reports. Note the person's details, incident description, and reporter's contact info, then follow two frameworks:

* **QUEST** (talking with the reporter): offer a **Q**uiet private space, check for **U**nsafe/dangerous situations, reflect **E**motions back neutrally, address **S**ecurity concerns about confidentiality, and build **T**rust — thank the reporter.
* **BIRDS** (talking with the reported person): state the **B**ehavior without judgment, the **I**mpact it had, let them **R**espond, **D**on't reassure or let them contact the reporter, and **S**et a behavioral modification plan; document the outcome afterward.

The template also has fields for a behavioral modification plan, consequences, appeal path, and the reported person's response.

**Quick links:**

* **Reporting form:** [https://forms.gle/3FsyDmG2rm1bF9PF7](https://forms.gle/3FsyDmG2rm1bF9PF7)
* **In-person reporting template**: [https://gitlab.com/otter-tech/coc-incident-response-workshop/blob/master/report-template-for-responders.pdf](https://gitlab.com/otter-tech/coc-incident-response-workshop/blob/master/report-template-for-responders.pdf)

<table data-view="cards"><thead><tr><th></th><th data-type="content-ref"></th></tr></thead><tbody><tr><td><a href="https://csidnet.gitbook.io/csidnet-handbook/~/revisions/S4xli0rUC6qCi6xp1pEC/readme/network-policies/code-of-conduct">Code of Conduct</a></td><td></td></tr><tr><td><a href="https://csidnet.gitbook.io/csidnet-handbook/~/revisions/3zLqRI4qlPXwMeCCYmCi/readme/network-policies/in-person-event-participation-guidelines">In-person Event Participation Guidelines</a></td><td></td></tr></tbody></table>

{% hint style="warning" %}
Co-hosts are responsible for sharing these policies with all participants before the event and for reinforcing them on the day.
{% endhint %}
